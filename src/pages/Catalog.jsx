...
import { changeSearchField, setSearchingStatus } from "../constants/actions";

const Catalog = () => {
  // redux
  const { searchField } = useSelector((state) => state.search);
  const { status: searchStatus } = useSelector((state) => state.search);
  ...
  const onSubmit = (e) => {
    ...
    //
    dispatch(setSearchingStatus("searching and loading"));
    dispatch(setSearchingStatus("done"));
